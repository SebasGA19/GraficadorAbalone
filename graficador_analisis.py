...
def correlacion(atipicos):
    if(atipicos==False):
        datos = abalone
    else:
        datos = abalone_limpio
    fig, ax = pyplot.subplots()
    cax = ax.matshow(datos.corr())
    fig.colorbar(cax)
    ax.set_xticklabels(['']+datos.columns, rotation='vertical')
...
tk.Label(root, text="Correlacion").place(x=10,y=400)
tk.Button(root, text="Graficar", command=lambda: correlacion(False)).place(x=10,y=430)
...
tk.Label(root,text= "Sesgo y Kurtosis").place(x=10,y=910)
tk.Label(root,textvariable=sesgo_v).place(x=10,y=940)
tk.Label(root,textvariable=kurtosis_c_v).place(x=10,y=970)
...
tk.Label(root, text="Correlacion").place(x=400,y=400)
tk.Button(root, text="Graficar", command=lambda: correlacion(True)).place(x=400,y=430)
...
tk.Label(root,text= "Sesgo y Kurtosis").place(x=400,y=910)
tk.Label(root,textvariable=sesgo_v).place(x=400,y=940)
tk.Label(root,textvariable=kurtosis_c_v).place(x=400,y=970)
